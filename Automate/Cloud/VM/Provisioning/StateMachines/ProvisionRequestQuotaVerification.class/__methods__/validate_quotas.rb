...
# 20-07-15 - Re-worked service_request_quota_validation.rb to support quota for Cloud (flavour) provisioning.
...
def query_prov_options(prov_option, options_array=[])
  if prov_option == :allocated_storage
    template = $evm.vmdb(:miq_template).find_by_id(@miq_request.get_option(:src_vm_id))
    if template
      log(:info, "Retrieving template #{template.name} allocated_disk_storage => #{template.allocated_disk_storage} from miq_request")
      options_array << template.allocated_disk_storage
    end
  elsif prov_option == :cores_per_socket
    flavor = $evm.vmdb(:flavor).find_by_id(@miq_request.get_option(:instance_type))
    if flavor
      log(:info, "Retrieving flavor #{flavor.name} cpus => #{flavor.cpus} from miq_request")
      options_array << flavor.cpus
    else
      log(:info, "Retrieving #{prov_option} => #{@miq_request.get_option(prov_option)} from miq_request")
      options_array << @miq_request.get_option(prov_option)
    end
  elsif prov_option == :vm_memory
    flavor = $evm.vmdb(:flavor).find_by_id(@miq_request.get_option(:instance_type))
    if flavor
      log(:info, "Retrieving flavor #{flavor.name} memory => #{flavor.memory} from miq_request")
...
def get_total_requested(prov_option)
...
def check_quotas(entity, quota_hash)
  ...
  log(:info, "entity_consumption: #{entity_consumption}")
  ...
  unless quota_max_storage.zero?
    if entity_consumption && (entity_consumption[:allocated_storage] / 1024**3 + quota_hash[:total_storage_requested] / 1024**3 > quota_max_storage)
      ...
      quota_hash[:quota_exceeded] = true
      quota_hash[entity_storage_reason] = "#{entity_type} - storage #{entity_consumption[:allocated_storage] / 1024**3}GB + requested #{quota_hash[:total_storage_requested] / 1024**3}GB &gt; quota #{quota_max_storage}GB"
    end
  end
  ...
  unless quota_warn_storage.zero?
    if entity_consumption && (entity_consumption[:allocated_storage] / 1024**3 + quota_hash[:total_storage_requested] / 1024**3 > quota_warn_storage)
      log(:info, "#{entity_type}: #{entity_name} current storage allocated: #{entity_consumption[:allocated_storage] / 1024**3}GB + requested: #{quota_hash[:total_storage_requested] / 1024**3}GB exceeds warn quota: #{quota_warn_storage}GB")
      quota_hash[:quota_warn_exceeded] = true
...
quota_hash[:total_cpus_requested]     = get_total_requested(:cores_per_socket)
quota_hash[:total_memory_requested]   = get_total_requested(:vm_memory)
quota_hash[:total_storage_requested]  = get_total_requested(:allocated_storage)
quota_hash[:total_storage_requested]  = get_total_requested(:provisioned_storage)
quota_hash[:total_vms_requested]      = get_total_requested(:number_of_vms)
...
quota_categories = ['quota_max_cpu', 'quota_max_memory', 'quota_max_storage', 'quota_max_vms']

manage_quotas_by_group, manage_quotas_by_owner = nil
# manage_quotas_by_owner = nil

# Check for owner\user quota tags
quota_categories.each { | c | manage_quotas_by_owner = true unless user.tags(c).blank? }

# Check for group quota tags
quota_categories.each { | c | manage_quotas_by_group = true unless group.tags(c).blank? }

# unless manage_quotas_by_owner.nil?
#   check_quotas(user, quota_hash)
# end
# unless manage_quotas_by_group.nil?
#   check_quotas(group, quota_hash)
# end

if manage_quotas_by_owner == true
  check_quotas(user, quota_hash)
else
  check_quotas(group, quota_hash)
end
...
if quota_hash[:quota_exceeded]
  quota_message = "Provisioning request denied due to the following quota limits:"
  ...
  quota_message = "Provisioning request warning due to the following quota thresholds:"
