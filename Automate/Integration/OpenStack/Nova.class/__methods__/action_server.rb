...
#puts conn.methods.sort.inspect
  ...
  #log(:info, "Got connection #{conn.class} #{conn.inspect}")    
	...
	case action
    when "start_server"
      raise "Fog error on selected action: #{action}" unless conn.server_action("#{instance_uuid}", { 'os-start' => nil })
      #raise "Fog error on selected action: #{action}" unless conn.start_server("#{instance_uuid}")
    when "stop_server"
      raise "Fog error on selected action: #{action}" unless conn.server_action("#{instance_uuid}", { 'os-stop' => nil })
      #raise "Fog error on selected action: #{action}" unless conn.server_action.stop_server("1","2")
    when "pause_server"
    ...
    when "reboot_server"
   	  raise "Fog error on selected action: #{action}" unless conn.server_action("#{instance_uuid}", { 'reboot' => { 'type' => 'HARD' }})
    else
